...
Param_table = cbind(fields,types)
...
Main <-
  ...
  # DEBUG : 
  # Pan to debug function
  # - Fix intern variables : 
            endpoint = 1
            limit = 50
  ...
  req = req_call(endpoint)
   ...
   if (is.logical(ascending.date) == TRUE) {
     ascending1 <-
       ifelse(ascending == FALSE, "&sort[]=date:desc", "&sort[]=date:asc")
   }
   ...
   if ((
     ...
     is.character(country) == TRUE | is.character(status) == TRUE
   )) {
  ...
  # Get RELIEFWEB - API-URL and add PARAMETEERS (Makes a "Kickstart"): 
    ...
    kickstart = ifelse(!is.null(ascending1),capture.output(cat(kickstart,ascending1,sep = "")),kickstart)
    kickstart = ifelse(!is.null(country1),capture.output(cat(kickstart,country1,sep = "")),kickstart)
    kickstart = ifelse(!is.null(date.start1),capture.output(cat(kickstart,date.start1,sep = "")),kickstart)
    kickstart = ifelse(!is.null(date.end1),capture.output(cat(kickstart,date.end1,sep = "")),kickstart)
    kickstart = ifelse(!is.null(disaster1),capture.output(cat(kickstart,disaster1,sep = "")),kickstart)
  ...
  # - endpoint = 1 -> reports -> title
  # - endpoint = 2 -> disasters -> name
  print(res_summary(res,type = endpoints[endpoint]))
...
# With DATE START :
res = Main(endpoint = endpoint,limit = 10,date.start = "2020-01-02",ascending.title = TRUE,ascending.date = FALSE)
```


```{r EXAMPLE_2, echo=TRUE}
endpoint = 1
value = c("cuba","hurrican")
field = c("title","title")
# operator = "OR"
...
PRINT_SEARCH(res = res,numero = 6,html = TRUE,txt = TRUE, namefile = "truc")

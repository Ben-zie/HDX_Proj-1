...
# With DATE START :
res = Main(endpoint = endpoint,limit = 10, ascending.date = TRUE)
```
...
x = req_call(endpoint = 1)
x = resp_body_json(req_perform(x), simplifyVector = TRUE)
x = as.data.frame(x)

res = Main(endpoint = 1,limit = 20,date.start = "2015-01-02",ascending.date = FALSE,value = c("Epidemic"),field = c("title"))

x = resp_body_json(req_perform(request(res$data[11,"href"])),simplifyVector = TRUE)
```